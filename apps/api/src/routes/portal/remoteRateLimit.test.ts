...
const app = () => { const a = new Hono(); a.use('*', async (c, next) => { c.set('portalAuth', { user: { id: 'portal-user', orgId: 'org-a', email: 'portal-user@example.test', name: 'Portal user', contactId: null, receiveNotifications: true, status: 'active' }, token: 'test-token', authMethod: 'bearer', timezone: 'UTC' }); await next(); }); a.use('*', portalRemoteStartRateLimit); a.post('/start', c => c.json({ ok: true })); return a; };
