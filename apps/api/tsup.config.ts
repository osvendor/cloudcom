...
export default defineConfig({
  ...
  entry: {
    index: 'src/index.ts',
    // BREEZE_ROLE=worker entrypoint (wave 3.5d-b, #4086) â€” same named-entry
  ...
  // OOMed (ERR_WORKER_OUT_OF_MEMORY) on heavy inferred types â€” e.g. the incident
  // feed's UNION ALL query builder â€” failing the build for ~150 bytes of unused
  // .d.cts. Disable it; the Dockerfile and runbook only ever run dist/*.cjs.
  dts: false,
  // All @breeze/* workspace packages are source-only (main â†’ src/index.ts), so bundle
  ...
  noExternal: [/^@breeze\//, /^@cloudcom\//, 'dotenv', 'mailparser', 'v9u-smb2'],
});
