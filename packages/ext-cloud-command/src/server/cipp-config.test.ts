...
const env = {
  CLOUDCOM_CIPP_ENABLED: 'true',
  CLOUDCOM_CIPP_ORIGIN: 'https://cipp.example.test',
  ...
  CLOUDCOM_CIPP_CLIENT_SECRET: 'fixture',
  CLOUDCOM_CIPP_SCOPE: 'api://33333333-3333-4333-8333-333333333333/.default',
};
describe('deployment-owned CIPP configuration', () => {
  ...
  it.each([
    'http://cipp.example.test',
    'https://user:pass@cipp.example.test',
    'https://cipp.example.test/api',
    'https://cipp.example.test/?token=a',
  ...
  it('binds mapping to backend identity while permitting a secret rotation', () => {
    ...
    expect(readCippDeployment({ ...env, CLOUDCOM_CIPP_CLIENT_SECRET: 'rotated' })!.identity).toBe(
      config.identity,
    );
    expect(
      readCippDeployment({ ...env, CLOUDCOM_CIPP_ORIGIN: 'https://replacement.example.test' })!.identity,
    ).not.toBe(config.identity);
  });
});
