...
export const cippResources = {
  ...
  groups: {
    ...
    fields: {
      ...
      securityEnabled: 'Security group',
      membershipRule: 'Membership rule',
    },
  },
  licenses: {
    endpoint: 'ListLicenses',
    id: 'skuId',
    fields: {
      License: 'License',
      CountUsed: 'Assigned',
  ...
  sites: {
    ...
    fields: {
      ...
      storageUsedInGigabytes: 'Used (GB)',
      storageAllocatedInGigabytes: 'Allocated (GB)',
      reportRefreshDate: 'Report date',
    },
  },
} as const;
...
function array(value: unknown): Record<string, unknown>[] {
  if (
    ...
    throw new CippError('invalid_provider_response');
  return value;
}
export function createCippProvider(fetch: GuardedFetch, config: CippDeployment) {
  ...
  async function read(endpoint: string, query: Record<string, string> = {}) {
    ...
    const token = z.object({ access_token: z.string().min(1).max(32768) }).safeParse(
      await json(
    ...
    return json(
      `${config.origin}/api/${endpoint}?${new URLSearchParams(query)}`,
      {
  ...
  return {
    async tenants() {
      ...
      return result.map((row) => {
        ...
        if (!parsed.success || seen.has(parsed.data.customerId.toLowerCase()))
          throw new CippError('invalid_tenant_response');
        seen.add(parsed.data.customerId.toLowerCase());
        return {
          id: parsed.data.customerId.toLowerCase(),
          name: parsed.data.displayName,
          domain: parsed.data.defaultDomainName.toLowerCase(),
        };
      });
    },
    async resource(resource: CippResource, tenantDomain: string) {
      ...
      const result = array(
        await read(spec.endpoint, {
          tenantFilter: tenantDomain,
          ...(resource === 'sites' ? { Type: 'SharePointSiteUsage' } : {}),
        }),
      );
      const seen = new Set<string>();
      const items = result.map((row) => {
        ...
        if (typeof id !== 'string' || !id || id.length > 512 || seen.has(id))
          throw new CippError('invalid_provider_response');
        if (resource !== 'sites' && !z.string().uuid().safeParse(id).success)
          throw new CippError('invalid_provider_response');
        seen.add(id);
      ...
      return {
        items,
        columns: Object.entries(spec.fields).map(([key, label]) => ({ key, label })),
        complete: true,
