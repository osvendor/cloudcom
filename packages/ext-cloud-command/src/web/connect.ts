...
export class CloudCommandConnectPage extends HTMLElement {
  ...
  private child: ThreeCxElement | null = null;
  ...
  private render() {
    ...
    if (!preserveChild) {
      ...
      if (this.selected === 'threecx' && this.isConnected && this.contextValue && this.api) {
        const child = document.createElement('cloudcommand-threecx-page') as ThreeCxElement;
        child.displayMode = 'configuration';
        ...
        this.child = child;
      } else if (this.selected === 'threecx') this.root.querySelector('[data-provider-panel]')!.textContent = 'Loading organization setup…';
    } else {
