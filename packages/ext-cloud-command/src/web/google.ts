...
export class CloudCommandGooglePage extends HTMLElement {
  ...
  private render() {
    ...
    this.root.querySelector('main')?.classList.toggle('storage', this.storageOpen || this.activityOpen);
    if (connected) this.root.querySelector('nav')?.insertAdjacentHTML('beforeend', `<button id="storage-tab" aria-current="${this.storageOpen}">Storage</button>${this.connection?.canManage ? `<button id="activity-tab" aria-current="${this.activityOpen}">Security activity</button>` : ''}`);
    ...
    this.root.querySelector('#refresh')?.addEventListener('click', () => { if (connected) { if (this.activityOpen) void this.loadActivity(true); else if (this.storageOpen) void this.loadStorage(true); else void this.loadPage(true); } else void this.loadConnection(); });
    ...
    this.root.querySelector('#storage-tab')?.addEventListener('click', () => { this.generation++; this.storageOpen = true; this.activityOpen = false; this.loading = false; this.pendingAction = null; this.pendingProfile = null; this.mailbox = null; this.memberGroup = null; this.render(); if (!this.storageLoaded) void this.loadStorage(true); });
    this.root.querySelector('#activity-tab')?.addEventListener('click', () => { this.generation++; this.activityOpen = true; this.storageOpen = false; this.loading = false; this.pendingAction = null; this.pendingProfile = null; this.mailbox = null; this.memberGroup = null; this.render(); if (!this.activityLoaded) void this.loadActivity(true); });
    ...
    this.root.querySelectorAll<HTMLButtonElement>('[data-kind]').forEach(button => button.addEventListener('click', () => {
      ...
      if (kind === this.kind && !this.storageOpen && !this.activityOpen) return;
      if (kind === this.kind && (this.storageOpen || this.activityOpen)) { this.generation++; this.storageOpen = false; this.activityOpen = false; this.loading = false; this.render(); return; }
