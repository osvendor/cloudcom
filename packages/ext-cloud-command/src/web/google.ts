...
type ActivitySource = 'login' | 'admin' | 'drive' | 'token';
type ActivityItem = { id: string; at: string; source: ActivitySource; actor: string | null; ip: string | null; events: string[] };
type ActivityPage = { asOf: string; items: ActivityItem[]; nextPageToken: string | null; partial: boolean; warning: string | null };
type TraceItem = { id: string; at: string; sender: string | null; recipient: string | null; subject: string | null; status: string; messageId: string | null; description: string | null };
type TracePage = { asOf: string; items: TraceItem[]; nextPageToken: string | null; partial: boolean; warning: string | null };
...
export class CloudCommandGooglePage extends HTMLElement {
  ...
  private activitySource: ActivitySource = 'login';
  private activityDays: 1 | 7 | 30 = 7;
  private activityRows: ActivityItem[] = [];
  private activityNext: string | null = null;
  private activityAsOf: string | null = null;
  private activityPartial = false;
  private activityWarning: string | null = null;
  private activityLoaded = false;
  private traceOpen = false;
  private traceDays: 1 | 7 | 30 = 7;
  private traceRows: TraceItem[] = [];
  private traceNext: string | null = null;
  private traceAsOf: string | null = null;
  private tracePartial = false;
  private traceWarning: string | null = null;
  private traceLoaded = false;
  private traceQuery = '';
  ...
  set context(input: unknown) {
    ...
    if (context.organizationId !== this.contextValue?.organizationId) {
      this.generation++; this.rows = []; this.nextToken = null; this.connection = null; this.error = ''; this.query = ''; this.pendingAction = null; this.pendingProfile = null; this.memberGroup = null; this.mailbox = null; this.storageOpen = false; this.storageRows = []; this.storageNext = null; this.storagePartial = false; this.storageWarning = null; this.storageLoaded = false; this.activityOpen = false; this.activityRows = []; this.activityNext = null; this.activityAsOf = null; this.activityPartial = false; this.activityWarning = null; this.activityLoaded = false; this.traceOpen = false; this.traceRows = []; this.traceNext = null; this.traceAsOf = null; this.tracePartial = false; this.traceWarning = null; this.traceLoaded = false; this.traceQuery = '';
  ...
  private async loadActivity(reset: boolean) {
    ...
    this.loading = true; this.error = ''; this.render();
    try {
      const page = await this.request<ActivityPage>(`/google/reports/activity?source=${this.activitySource}&days=${this.activityDays}${token && this.activityAsOf ? `&pageToken=${encodeURIComponent(token)}&asOf=${encodeURIComponent(this.activityAsOf)}` : ''}`);
      if (generation !== this.generation || !this.activityOpen) return;
      this.activityRows = reset ? page.items : [...this.activityRows, ...page.items];
      this.activityNext = page.nextPageToken;
      this.activityAsOf = page.asOf;
      this.activityPartial = (reset ? false : this.activityPartial) || page.partial;
      this.activityWarning = page.warning ?? (reset ? null : this.activityWarning);
      this.activityLoaded = true;
    } catch (error) {
      if (generation === this.generation && this.activityOpen) this.error = error instanceof Error ? error.message : 'Could not load Google activity.';
    }
  ...
  private async loadTrace(reset: boolean) {
    ...
    this.loading = true; this.error = ''; this.render();
    try {
      const page = await this.request<TracePage>(`/google/reports/trace?days=${this.traceDays}${token && this.traceAsOf ? `&pageToken=${encodeURIComponent(token)}&asOf=${encodeURIComponent(this.traceAsOf)}` : ''}`);
      if (generation !== this.generation || !this.traceOpen) return;
      this.traceRows = reset ? page.items : [...this.traceRows, ...page.items];
      this.traceNext = page.nextPageToken;
      this.traceAsOf = page.asOf;
      this.tracePartial = (reset ? false : this.tracePartial) || page.partial;
      this.traceWarning = page.warning ?? (reset ? null : this.traceWarning);
      this.traceLoaded = true;
    } catch (error) {
      if (generation === this.generation && this.traceOpen) this.error = error instanceof Error ? error.message : 'Could not load Gmail audit trace.';
    }
    if (generation === this.generation && this.traceOpen) { this.loading = false; this.render(); }
  }
  ...
  private render() {
    ...
    if (this.activityOpen) this.root.querySelector('main')?.insertAdjacentHTML('beforeend',
      `<section class="members"><div class="members-head"><div><h2>Security activity</h2><p>Google Admin Reports · read-only</p></div><label>Source<select id="activity-source">${(['login', 'admin', 'drive', 'token'] as const).map(source => `<option value="${source}" ${source === this.activitySource ? 'selected' : ''}>${source}</option>`).join('')}</select></label><label>Window<select id="activity-days"><option value="1" ${this.activityDays === 1 ? 'selected' : ''}>1 day</option><option value="7" ${this.activityDays === 7 ? 'selected' : ''}>7 days</option><option value="30" ${this.activityDays === 30 ? 'selected' : ''}>30 days</option></select></label><button id="load-activity" ${this.loading ? 'disabled' : ''}>Load</button></div><p class="note">Requires admin.reports.audit.readonly in the existing delegation grant. Results depend on Google logging, retention, edition and administrator privileges; an empty page does not prove no activity.</p>${this.activityWarning ? `<p role="status">${escape(this.activityWarning)}</p>` : ''}${this.activityLoaded ? `<p role="status">${this.activityPartial ? 'Partial records' : this.activityNext ? 'More pages available' : 'Loaded window'} · ${this.activityDays} day(s)</p><div class="table"><table><thead><tr><th>Time</th><th>Actor</th><th>Event</th><th>IP</th></tr></thead><tbody>${this.activityRows.map(row => `<tr><td>${escape(row.at)}</td><td>${escape(row.actor ?? 'Unavailable')}</td><td>${escape(row.events.join(', ') || 'Unavailable')}</td><td>${escape(row.ip ?? 'Unavailable')}</td></tr>`).join('')}</tbody></table></div>${!this.activityRows.length ? '<p>No activity records returned for this source and window.</p>' : ''}${this.activityNext ? `<button id="more-activity" ${this.loading ? 'disabled' : ''}>Load more</button>` : ''}` : '<p>Choose a source and window, then load activity.</p>'}</section>`);
    if (this.traceOpen) {
      const query = this.traceQuery.toLowerCase();
      const shown = this.traceRows.filter(row => !query || [row.sender, row.recipient, row.status].some(value => value?.toLowerCase().includes(query)));
      this.root.querySelector('main')?.insertAdjacentHTML('beforeend',
        `<section class="members"><div class="members-head"><div><h2>Message trace</h2><p>Google Gmail audit · read-only</p></div><label>Window<select id="trace-days"><option value="1" ${this.traceDays === 1 ? 'selected' : ''}>1 day</option><option value="7" ${this.traceDays === 7 ? 'selected' : ''}>7 days</option><option value="30" ${this.traceDays === 30 ? 'selected' : ''}>30 days</option></select></label><button id="load-trace" ${this.loading ? 'disabled' : ''}>Load</button></div><p class="note">Audit events are not final delivery proof. Search filters loaded results only; more pages may be available. Requires admin.reports.audit.readonly.</p><label class="search">Search loaded sender, recipient or status<input id="trace-search" value="${escape(this.traceQuery)}"></label>${this.traceWarning ? `<p role="status">${escape(this.traceWarning)}</p>` : ''}${this.traceLoaded ? `<p role="status">${this.tracePartial ? 'Partial records' : this.traceNext ? 'More pages available' : 'Loaded window'} · ${this.traceDays} day(s)</p><div class="table"><table><thead><tr><th>Time</th><th>Sender</th><th>Recipient</th><th>Subject</th><th>Status</th></tr></thead><tbody>${shown.map(row => `<tr><td>${escape(row.at)}</td><td>${escape(row.sender ?? 'Unavailable')}</td><td>${escape(row.recipient ?? 'Unavailable')}</td><td>${escape(row.subject ?? 'Unavailable')}</td><td>${escape(row.status)}</td></tr>`).join('')}</tbody></table></div>${!shown.length ? '<p>No audit events in the loaded results.</p>' : ''}${this.traceNext ? `<button id="more-trace" ${this.loading ? 'disabled' : ''}>Load more</button>` : ''}` : '<p>Choose a window, then load trace.</p>'}</section>`);
    }
    this.root.querySelector('#refresh')?.addEventListener('click', () => { if (connected) { if (this.traceOpen) void this.loadTrace(true); else if (this.activityOpen) void this.loadActivity(true); else if (this.storageOpen) void this.loadStorage(true); else void this.loadPage(true); } else void this.loadConnection(); });
    ...
    this.root.querySelector('#storage-tab')?.addEventListener('click', () => { this.generation++; this.storageOpen = true; this.activityOpen = false; this.traceOpen = false; this.loading = false; this.pendingAction = null; this.pendingProfile = null; this.mailbox = null; this.memberGroup = null; this.render(); if (!this.storageLoaded) void this.loadStorage(true); });
    this.root.querySelector('#activity-tab')?.addEventListener('click', () => { this.generation++; this.activityOpen = true; this.storageOpen = false; this.traceOpen = false; this.loading = false; this.pendingAction = null; this.pendingProfile = null; this.mailbox = null; this.memberGroup = null; this.render(); if (!this.activityLoaded) void this.loadActivity(true); });
    this.root.querySelector('#trace-tab')?.addEventListener('click', () => { this.generation++; this.traceOpen = true; this.activityOpen = false; this.storageOpen = false; this.loading = false; this.pendingAction = null; this.pendingProfile = null; this.mailbox = null; this.memberGroup = null; this.render(); if (!this.traceLoaded) void this.loadTrace(true); });
    this.root.querySelector('#load-trace')?.addEventListener('click', () => { this.traceDays = Number(this.root.querySelector<HTMLSelectElement>('#trace-days')?.value ?? 7) as 1 | 7 | 30; void this.loadTrace(true); });
    this.root.querySelector('#more-trace')?.addEventListener('click', () => void this.loadTrace(false));
    this.root.querySelector<HTMLInputElement>('#trace-search')?.addEventListener('input', event => { this.traceQuery = (event.target as HTMLInputElement).value; this.render(); this.root.querySelector<HTMLInputElement>('#trace-search')?.focus(); });
    this.root.querySelector('#load-activity')?.addEventListener('click', () => { this.activitySource = this.root.querySelector<HTMLSelectElement>('#activity-source')?.value as ActivitySource ?? 'login'; this.activityDays = Number(this.root.querySelector<HTMLSelectElement>('#activity-days')?.value ?? 7) as 1 | 7 | 30; void this.loadActivity(true); });
    this.root.querySelector('#more-activity')?.addEventListener('click', () => void this.loadActivity(false));
    ...
    this.root.querySelectorAll<HTMLButtonElement>('[data-kind]').forEach(button => button.addEventListener('click', () => {
      ...
      if (kind === this.kind && !this.storageOpen && !this.activityOpen && !this.traceOpen) return;
      if (kind === this.kind && (this.storageOpen || this.activityOpen || this.traceOpen)) { this.generation++; this.storageOpen = false; this.activityOpen = false; this.traceOpen = false; this.loading = false; this.render(); return; }
