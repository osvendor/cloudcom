...
export class CloudCommandMicrosoftPage extends HTMLElement {
  ...
  private renderDrawer(canManage: boolean): string {
    ...
    if (this.detailKind === 'read') {
      ...
      const accessChanged = typeof this.userDraft.accountEnabled === 'boolean' && this.userDraft.accountEnabled !== (record.accountEnabled === true);
      ...
      content = `<div class="drawer-body"><p class="meta">User ID: ${esc(detail.id)}</p><section class="drawer-section"><h3>Profile</h3><div class="field-grid">${field('displayName', 'Name')}${field('givenName', 'Given name')}${field('surname', 'Surname')}${field('department', 'Department')}${field('jobTitle', 'Job title')}${field('officeLocation', 'Office location')}</div></section><section class="drawer-section"><h3>Account access</h3><div class="user-field"><label class="check"><input id="user-account-enabled" type="checkbox" ${enabled === true ? 'checked' : ''} ${this.busy || !canManage ? 'disabled' : ''}> Account enabled</label>${canManage ? `<button class="secondary" type="button" data-user-save="accountEnabled" ${this.busy || !accessChanged ? 'disabled' : ''}>Save access</button>` : ''}</div></section>${canManage ? `<section class="drawer-section"><h3>Security</h3><div class="actions"><button class="secondary" id="user-password-reset-start" ${this.busy ? 'disabled' : ''}>Reset password</button><button class="secondary" id="user-sessions-revoke-start" ${this.busy ? 'disabled' : ''}>Sign out of all sessions</button></div>${securityConfirmation}${passwordDisplay}</section>` : ''}</div><footer class="drawer-footer">${canManage ? '' : '<p class="read-only">An organization administrator can edit this user.</p>'}${feedback}</footer>`;
    } else {
