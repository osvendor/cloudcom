...
function extra(tab: DetailTab, user: ThreeCxDetail["user"]) {
  if (tab === "schedule")
    return `<section><h3>Schedule</h3>${note("Schedule data is not included in the current safe PBX detail projection.")}</section>`;
  const fields =
    tab === "talk"
      ? [
