...
| --- | --- | --- | --- |
| RustDesk quiet unattended profile | Candidate: `scripts/cloudcom/rustdesk/common.ps1`, `repair.ps1`, `repair.Tests.ps1`; default quiet-state detection and repair. | Keep native monitor/script contracts and validate stock RustDesk shortcut targets and tray CLI on upgrades. No Breeze MSI or RustDesk fork; does not suppress active-session UI. | Eighteen Linux Pester checks passed. Live OS-TEST quiet/reboot acceptance pending. Roll back policy/script versions, preserve DPAPI credentials. |
