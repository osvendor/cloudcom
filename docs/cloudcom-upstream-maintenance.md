...
| --- | --- | --- | --- |
| Runner and workflow routing | Implemented in `.github/workflows/`, `.github/scripts/check-cloudcom-runner.sh`, and `.github/actionlint.yaml`. `typecheck`, `test-api` (8 shards), `test-web` (4 shards), and `integration-test` (16 shards) are pinned to hosted `ubuntu-24.04`; Type Check has a 30-minute deadline; see [CI guide](cloudcom-ci.md). | Preserve hosted routing for external PRs and privileged/native jobs, full checkout, contributor approval, and required CI Success semantics. The assigned 8 GB local worker cannot satisfy Type Check's 12 GB heap, and it would serialize the 28 test shards; public `ubuntu-24.04` is documented with 16 GB RAM. | Actionlint; runner-routing contract proves these four jobs stay hosted, retains their area gates and shard matrices, and protects Type Check's timeout; fresh full CI on integration head. |
