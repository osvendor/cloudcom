...
The extension has a read-only Call Log candidate for connections explicitly scoped to the full PBX. It uses a fixed 3CX report function, validates a range of no more than 31 days, reads at most 100 events per page, projects only approved call metadata, audits each scoped read, and labels CSV export as matching loaded results. It does not load call events until the operator searches. The operator can filter at most 1,000 loaded events; filtering and CSV export never imply that the complete PBX history was searched. A department-scoped connection receives `report_scope_unverified` before decrypting a credential or calling the provider: the retained Cloud Command report query had no explicit department predicate, so provider role constraints alone must not be assumed to isolate customer call records. A continuation is offered only when the PBX supplies a next link for the exact same origin, report-function path and fixed query, with an offset exactly one 100-row page ahead. The app constructs the next request itself and never follows the provider URL. It issues a ten-minute HMAC cursor tied to the connection version, organization, date range, and exact next offset; a bare or altered `$skip` cannot request a later page. A full page without a valid continuation remains labelled incomplete. This is candidate code, not a live PBX paging or report-isolation proof. Any future department support requires a demonstrated provider-side boundary before the gate is relaxed.
...
Before claiming complete pagination, verify against an authorized PBX report fixture that `GetCallLogData` returns a stable order and that successive pages have no duplicate or missing call IDs. The retained source used `$skip`, while 3CX's public Configuration API documentation does not prove this function's ordering. The candidate's bounded continuation intentionally reports only loaded results. Before enabling department reporting, prove the service principal's selected department and role actually constrain this report on a PBX containing at least two departments, including calls crossing them. If provider-side isolation cannot be shown, require a documented complete row-level department attribution before projecting any record; caller/callee number matching is not a sufficient tenant boundary. Keep the department gate closed when proof is missing.
